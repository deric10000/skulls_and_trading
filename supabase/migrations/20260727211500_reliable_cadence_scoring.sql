-- Reliable Cadence Scoring
-- user_state remains the source of truth. These tables are normalized,
-- rebuildable projections and idempotent execution/result records.

create extension if not exists pgcrypto;
create extension if not exists pg_cron with schema pg_catalog;
create extension if not exists pg_net with schema extensions;

-- Conviction history is portfolio-scoped. The prior key allowed the same
-- strategy+ticker in two books to overwrite one another.
alter table public.conviction_snapshots
  add column if not exists portfolio_id text not null default '';
alter table public.conviction_snapshots
  drop constraint if exists conviction_snapshots_user_id_strategy_id_ticker_as_of_key;
alter table public.conviction_snapshots
  add constraint conviction_snapshots_user_portfolio_strategy_ticker_as_of_key
...
create index if not exists strategy_check_schedules_due_idx
  on public.strategy_check_schedules (next_due_at)
  where enabled;
...
drop policy if exists "market_symbol_subscriptions_select_own"
  on public.market_symbol_subscriptions;
create policy "market_symbol_subscriptions_select_own"
  on public.market_symbol_subscriptions for select
  using (auth.uid() = user_id);

drop policy if exists "strategy_check_schedules_select_own"
  on public.strategy_check_schedules;
create policy "strategy_check_schedules_select_own"
  on public.strategy_check_schedules for select
  using (auth.uid() = user_id);

drop policy if exists "strategy_check_runs_select_own"
  on public.strategy_check_runs;
create policy "strategy_check_runs_select_own"
  on public.strategy_check_runs for select
  using (auth.uid() = user_id);

drop policy if exists "strategy_check_state_select_own"
  on public.strategy_check_state;
create policy "strategy_check_state_select_own"
  on public.strategy_check_state for select
  using (auth.uid() = user_id);

drop policy if exists "strategy_check_ticker_state_select_own"
  on public.strategy_check_ticker_state;
create policy "strategy_check_ticker_state_select_own"
  on public.strategy_check_ticker_state for select
  using (auth.uid() = user_id);

drop policy if exists "strategy_check_latest_results_select_own"
  on public.strategy_check_latest_results;
create policy "strategy_check_latest_results_select_own"
  on public.strategy_check_latest_results for select
  using (auth.uid() = user_id);
drop policy if exists "strategy_check_combined_latest_results_select_own"
  on public.strategy_check_combined_latest_results;
...
begin
  -- Serialize every projection mutation so concurrent accounts cannot race the
  -- global distinct-symbol admission check.
  ...
  authoritative_symbols as (
    select ticker
    from public.market_symbol_subscriptions
  ...
  where schedule.user_id = p_user_id
    and not exists (
      ...
      cross join lateral (
        select public.normalize_check_cadence(strategy->>'checkInterval') as cadence
        union
...
create or replace function public.reconcile_strategy_first_check(
  p_strategy_id text
)
returns void
language plpgsql
...
begin
  ...
  where schedule.enabled
    and schedule.next_due_at <= p_cycle_as_of
  ...
  with claimable as (
    ...
    where run.cycle_key = p_cycle_key
      ...
      and (
        ...
        or (
          run.status = 'running'
          and run.claimed_at < now() - interval '10 minutes'
        )
      )
...
begin
  select * into claimed
  from public.strategy_check_runs
  where id = p_run_id
  for update;

  if not found then
    raise exception 'Unknown strategy check run';
  end if;
  if claimed.status = 'complete' then
    return false;
  end if;
  ...
  select
    ...
    claimed.strategy_id,
    (claimed.cycle_as_of at time zone 'America/New_York')::date,
    snapshot.holdings_market_value,
    snapshot.cost_basis,
    snapshot.cash_available,
    snapshot.total_value,
    snapshot.open_pnl,
    snapshot.open_pnl_pct,
  ...
  values (
    claimed.user_id, claimed.strategy_id, claimed.cadence,
...
begin
  ...
  where name = 'conviction_cycle_function_url'
  limit 1;

  select decrypted_secret into secret
  from vault.decrypted_secrets
  where name = 'conviction_cycle_internal_secret'
  limit 1;
...
-- Backfill projections for workspaces that predate this migration.
select public.refresh_scoring_projections(user_id)
from public.user_state;
