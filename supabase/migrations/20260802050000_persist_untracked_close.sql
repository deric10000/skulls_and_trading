-- Persist import untrackedClose on portfolio_transactions so historical
-- reconstruction can honor intentional brokerage closes that begin from a
-- zero-accounted holding, instead of treating them as replay corruption.
-- Forward-only.

alter table public.portfolio_transactions
  add column if not exists untracked_close boolean not null default false;
...
declare
  ...
  source_row integer;
begin
  -- Allow large-but-valid import batches (up to 5000 rows) to finish.
  ...
  -- Validate distinct time zones once (pg_timezone_names is expensive per row).
  if exists (
    select 1
    from (
      select distinct nullif(trim(value->>'timeZone'), '') as tz
      from jsonb_array_elements(p_transactions) rows(value)
    ) zones
    where zones.tz is null
      or length(zones.tz) > 64
      or not exists (
        select 1 from pg_catalog.pg_timezone_names names
        where names.name = zones.tz
      )
  ) then
    perform public.raise_portfolio_import_error(
      'invalid_opening_boundary',
      jsonb_build_object('code', 'invalid_date_timezone')
    );
  end if;
  ...
  if (
    ...
    from (
      ...
      where source->>'id' <> p_portfolio_id
        and coalesce((holding->>'shares')::numeric, 0) > 0
      union all
      select holding->>'ticker'
      from jsonb_array_elements(coalesce(p_portfolio->'holdings', '[]'::jsonb)) holding
      where coalesce((holding->>'shares')::numeric, 0) > 0
    ) active_tickers
  ) > 40 then
    perform public.raise_portfolio_import_error(
  ...
  begin
    ...
    insert into public.portfolio_transactions (
      ...
      action_class, strategy_ids, zone_hints, untracked_close
    )
    select
      ...
      '[]'::jsonb, '[]'::jsonb,
      coalesce((tx_row->>'untrackedClose')::boolean, false)
...
alter function public.commit_portfolio_transaction_batch(text, integer, jsonb, jsonb, jsonb)
  set statement_timeout = '120s';


drop function if exists public.read_historical_reconstruction_chunk(uuid, text, integer);
...
as $$
  select tx.id, tx.portfolio_id, tx.kind, tx.transaction_type, tx.ticker,
    ...
    tx.shares_after, tx.cash_before, tx.cash_after, tx.untracked_close
