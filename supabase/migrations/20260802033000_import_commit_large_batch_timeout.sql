-- Large brokerage imports (hundreds of executed rows) were timing out under
-- the default statement_timeout because every row re-queried pg_timezone_names.
-- Validate distinct time zones once, raise the function timeout, and keep the
-- rest of the commit contract unchanged. Forward-only.
