...
- Font family: **IBM Plex Sans** (primary), with `system-ui, -apple-system,
  ...
  Sans is self-hosted as exact Latin WOFF2 files (weights 400/500/600/700) plus
  IBM Plex Mono 400 via `@font-face` / `font-display: swap` in
  [`src/index.css`](src/index.css), and set on `:root`; its slightly technical,
  engineered character suits the command-deck tone. Do not restore a
  third-party font stylesheet or synthesize a missing weight.
...
- **`Radio` / `.radio`** — the mutually exclusive counterpart to `Checkbox` (`src/components/Radio.tsx`). It keeps the same 28×28 blue info treatment with a circular selected indicator. Use a stacked `radiogroup` for short either/or decisions; pass `decorative` when the parent row owns the click and radio semantics.
- **`Tooltip`** — the only explanatory popover. Desktop/tablet uses hover/focus; mobile normally uses the stock bottom sheet. Pass `desktopOnly` for icon actions whose mobile tap must perform the action immediately instead of opening help.
...
- **`RowMessage` / `.row-message`** — quiet validation copy inside a table or
  form row. It matches the regular-weight, muted informational typography used
  by Forge picker guidance: no pill, border, fill, icon, or bold/link treatment.
  `error` uses `--negative` and blocks submission or update; `warning` uses
  `--warning` and allows the user to proceed. Place it directly beneath the
  affected field for ordinary forms. In tabular rows, use a full-width message
  region beneath all cells; multiple messages stack as separate lines. Messages
  remain in normal flow so the owning row grows and pushes later rows down;
  never vertically offset sibling cells or overlay/truncate messages to preserve
  a fixed row height.
